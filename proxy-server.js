...
app.post("/query", async (req, res, next) => {
  ...
  if (query.startsWith("sample:")) {
    try {
      ...
      visualizationNodesAndEdges = JSON.parse(
        fs.readFileSync(
          path.join(__dirname, "samples", `${sample}.json`),
          "utf8"
        )
      );
    } catch (err) {
  ...
  const visualizationNodesAndEdgesPrettyfiedJSon = JSON.stringify(
    visualizationNodesAndEdges,
    null,
    2
  );
...
app.listen(config.port, () =>
  console.log(`Simple Gremlin proxy-server listening on port ${config.port}!`)
);
